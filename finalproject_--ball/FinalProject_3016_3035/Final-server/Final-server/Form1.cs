...
namespace Final_server
{
    public partial class Form1 : Form
    {
        ...
        //接受客戶連線要求的程式(如同電話總機)，針對每一客戶建立一個連線，以及獨立執行緒
        ...
        private void Listen()
        {
            ...
            while (true)            //持續監聽客戶傳來的訊息
            {
                ...
                {
                    byte[] B = new byte[2024];    //建立接收資料用的陣列，長度須大於可能的訊息
                    ...
                    switch (Cmd)                                           //依據命令碼執行功能
                    {
                        case "0":                           //有新使用者上線 : 新增使用者到名單中
                            ...
                            else
                            {
                                ...
                                B = Encoding.Default.GetBytes(reply);        //訊息轉譯為byte陣列
                            ...
                            break;
                        case "9":
                            HT.Remove(Str);                //移除使用者名稱為Name的連續物件
